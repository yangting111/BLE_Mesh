...
class BLEMesh_Network_PDU(Packet):
    name = "BLEMesh_Network_PDU"
    fields_desc = [BitEnumField("IVI", 0, 1,{0: "ACCEPT", 1: "TRANSMIT"}),
                   BitField("NID", 0, 7), 
                   BitEnumField("CTL", 1, 1, {0: "Access Message and NetMIC 32bit", 1: "Control Message and NetMIC 64bit"}),
                   BitField("TTL", 0, 7),
                   ThreeBytesField("SEQ",20),
                   ShortField("SRC", 0),
                   ShortField("DST", 0),
                   ConditionalField(IntField("NetMIC_32", 0),
                        lambda pkt: pkt.CTL == 0), 
...
class Seg_Ack_Message(Packet):
    name = "Segmented_Ack_Message"
    fields_desc = [
        BitField("SEG", 1, 1),
        BitField("Opcode", 0, 7),
        BitField("OBO", 0, 1),
        BitField("SeqZero", 0, 13),
        BitField("RFU", 0, 5),
        BitField("AckSegN", 0, 5)
    ]
##################################
#     BLEMesh_Upper_Transport    #
##################################

class Upper_Transport_Access(Packet):

    name = "Upper_Transport_Access"
    fields_desc = [
        StrLenField("EncAccessMessage", None),
        ConditionalField(StrFixedLenField("TransMIC", b'\x00'*4, 4), lambda pkt: pkt.SZMIC == 0),
        ConditionalField(StrFixedLenField("TransMIC", b'\x00'*8, 8), lambda pkt: pkt.SZMIC == 1)
    ]
...
class PATH_REQUEST(Packet):
    name = "PATH_REQUEST"
    fields_desc = [
        BitField("OnBehalf", 0, 1),
        BitField("MetricType", 0, 3),
        BitField("Lifetime", 0, 2),
        BitField("DiscoveryInterval", 0, 1),
        BitField("Prohibited", 0, 1),

        ByteField("ForwardingNumber", 0),
        ByteField("PathMetric", 0),
        LEShortField("Destination", 0), 
        StrLenField("PathOriginAddrRange", None), 
        StrLenField("DependentOriginAddrRange", None) 
    ]

class PATH_REPLY(Packet):
    name = "PATH_REPLY"
    fields_desc = [
        BitField("UnicastDestination", 0, 1),
        BitField("OnBehalfOfDependentTarget", 0, 1),
        BitField("ConfirmationRequest", 0, 1),
        BitField("Prohibited", 0, 5),

        LEShortField("PathOrigin", 0),
        ByteField("ForwardingNumber", 0), 
        StrLenField("TargetAddrRange", None), 
        StrLenField("DependentTargetAddrRange", None) 
    ]

class PATH_CONFIRMATION(Packet):
    name = "PATH_CONFIRMATION"
    fields_desc = [
        LEShortField("PathOrigin", 0),
        LEShortField("PathTarget", 0),          
    ]

class PATH_ECHO_REQUEST(Packet):
    name = "PATH_ECHO_REQUEST"


class PATH_ECHO_REPLY(Packet):
    name = "PATH_ECHO_REPLY"
    fields_desc = [
        LEShortField("Destination", 0)
    ]

class DEPENDENT_NODE_UPDATE(Packet):
    name = "DEPENDENT_NODE_UPDATE"
    fields_desc = [
        BitField("Type", 0, 1),
        BitField("Prohibited", 0, 7),
        LEShortField("PathEndpoint", 0), 
        StrLenField("AddrRange", None), 
    ]
class PATH_REQUEST_SOLICITATION(Packet):
...
class BLEMesh_Unprovisioned_Beacon(Packet):
...
class BLEMesh_Secure_Network_Beacon(Packet):

    name = "BLEMesh Secure Network Beacon" 

    fields_desc = [
                  ByteEnumField("Beacon_Type", 1, {0:"Unprovisioned Device Beacon",1:"Secure Network Beacon"}),
...
class BLEMesh_Private_Beacon(Packet):
    name = "BLEMesh Private Beacon"
    fields_desc = [
        ByteEnumField("Beacon_Type", 2, {0:"Unprovisioned Device Beacon",1:"Secure Network Beacon",2:"Private Beacon"}),
        StrFixedLenField("Random", b'\x00'*13, 13),
        StrFixedLenField("ObfuscatedData", b'\x00'*5, 5),  
        StrFixedLenField("AuthTag", b'\x00'*8, 8)
    ]
...
bind_layers(BLEMesh_Message, Friend_Poll, Opcode = 1)
bind_layers(BLEMesh_Message, Friend_Update, Opcode = 2)
bind_layers(BLEMesh_Message, Friend_Request, Opcode = 3)
bind_layers(BLEMesh_Message, Friend_Offer, Opcode = 4)
bind_layers(BLEMesh_Message, Friend_Clear, Opcode = 5)
bind_layers(BLEMesh_Message, Friend_Clear_Confirm, Opcode = 6)
bind_layers(BLEMesh_Message, Friend_Subscription_List_Add, Opcode = 7)
bind_layers(BLEMesh_Message, Friend_Subscription_List_Remove, Opcode = 8)
bind_layers(BLEMesh_Message, Friend_Subscription_List_Confirm, Opcode = 9)
bind_layers(BLEMesh_Message, Heartbeat, Opcode = 10)
bind_layers(BLEMesh_Message, PATH_REQUEST, Opcode = 11)
bind_layers(BLEMesh_Message, PATH_REPLY, Opcode = 12)
bind_layers(BLEMesh_Message, PATH_CONFIRMATION, Opcode = 13)
bind_layers(BLEMesh_Message, PATH_ECHO_REQUEST, Opcode = 14)
bind_layers(BLEMesh_Message, PATH_ECHO_REPLY, Opcode = 15)
bind_layers(BLEMesh_Message, DEPENDENT_NODE_UPDATE, Opcode = 16)
bind_layers(BLEMesh_Message, PATH_REQUEST_SOLICITATION, Opcode = 17)
